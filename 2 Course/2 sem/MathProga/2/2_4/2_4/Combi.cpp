...
namespace combi
{
    ...
    {
        // Инициализация параметров и создание генераторов сочетаний и перестановок
        this->n = n;
        this->m = m;
    ...
    {
        this->na = 0;            // Сброс счетчика
        this->cgen->reset();      // Сброс генератора сочетаний
        this->pgen->reset();      // Сброс генератора перестановок
        this->cgen->getfirst();   // Получение первого сочетания
    }
    // Получение первой размещения и возврат размера
    short accomodation::getfirst()
    {
        short rc = (this->n >= this->m) ? this->m : -1;
        // Если размер размещения корректен
        if (rc > 0)
        {
            // Заполнение массива sset текущим сочетанием
            for (int i = 0; i <= this->m; i++)
                this->sset[i] = this->cgen->sset[this->pgen->ntx(i)];
        };
        return rc;
    };
    // Получение следующего размещения и возврат размера
    short accomodation::getnext()
    {
        short rc;
        this->na++;// Увеличение счетчика размещений
        // Если есть следующая перестановка
        if ((this->pgen->getnext()) > 0)
            rc = this->getfirst();
        // Если следующей перестановки нет, но есть следующее сочетание
        else  if ((rc = this->cgen->getnext()) > 0)
        {
            this->pgen->reset();  // Сброс генератора перестановок
            rc = this->getfirst();  // Получение первой перестановки
        };
        return rc;
    };
    // Получение элемента размещения по индексу 'i'
    ...
    // Вычисление и возврат общего числа размещений
