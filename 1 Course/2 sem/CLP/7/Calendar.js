...
function Calendar({ Component }) {
  class Calendar extends React.Component {
    ...
    // CalendarBody отображает календарь с днями текущего месяца и позволяет выбирать даты. 
        <>
        {/* Метод  передается в качестве свойства, чтобы можно было изменять текущую дату  */}
...
class CalendarHead extends React.Component {
  ...
  render() {
    return (
      <>
      {/* Название текущего месяца  отображаются с помощью свойств объекта Date */}
        {this.monthNames[this.state.date.getMonth()]}
        {/* Название текущего года */}
        {this.state.date.getFullYear()}
